...
function solve(items) {
  ...
  for (const guard in items) {
    ...
    const mins = sleeps
      .map(sleep => sleep.wake - sleep.fall)
      .reduce((sum, i) => i + sum, 0);

    maxMins.push({ guard, mins });
  }

  maxMins.sort((a, b) => a.mins - b.mins);

  const maxGuardId = maxMins.pop().guard;
  const sleeps = items[maxGuardId];
...
assert(solve(parsed), { id: "#10", min: "24" });
